...
public class ExampleApp : MonoBehaviour
{
    ...
    public async void ReadEnvironment2Ds()
    {
        ...
        switch (webRequestResponse)
        {
            case WebRequestData<List<Environment2D>> dataResponse:
                ...
                // Laad de Environment2D-objecten in de UI
                LoadEnvironmentsUI(environment2Ds);
                break;
            case WebRequestError errorResponse:
                string errorMessage = errorResponse.ErrorMessage;
    ...
    private void LoadEnvironmentsUI(List<Environment2D> environments)
    {
        ...
        {
            //GameObject item = Instantiate(environmentItemPrefab, environmentListContainer);
            //item.GetComponent<EnvironmentItemUI>().Initialize(environment);
        }
    }
